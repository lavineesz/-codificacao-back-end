...
// 1. LISTENERS DE PROCESSO GLOBAL (REDE DE SEGURANÇA FINAL)
...
next(erro); // Encaminha para o Middleware Global
}
});
// Rota com erro em operação assíncrona
app.get('/erro-assincrono', async (req, res, next) => {
try {
await Promise.reject(new Error('Erro na consulta com banco de dados externo.'));
} catch (erro) {
next(erro);
}
});
